...
class GraphDataset:
    ...
    def get_train_w_labels(self, label):
         
        inds_to_choose = (self.Y[self.train_index] == label).nonzero(as_tuple=True)[0]
        inds_to_choose = np.array(inds_to_choose,dtype = np.int64)
        chosen = np.array(self.train_index[inds_to_choose],dtype=np.int64)
        train_graphs = [] 
        train_explanations = [] 
        for idx in chosen:
            train_graphs.append(self.graphs[idx])
            train_explanations.append(self.explanations[idx])

        return train_graphs, train_explanations
    ...
    def get_test_w_labels(self, label):
        assert self.test_index is not None, 'test_index is None'
        inds_to_choose = (self.Y[self.test_index] == label).nonzero(as_tuple=True)[0]
        inds_to_choose = np.array(inds_to_choose,dtype = np.int64)
        chosen = np.array(self.test_index[inds_to_choose],dtype=np.int64)
        test_graphs = [] 
        test_explanations = [] 
        for idx in chosen:
            test_graphs.append(self.graphs[idx])
            test_explanations.append(self.explanations[idx])
        
        return test_graphs, test_explanations
        #return self.graphs[chosen], self.explanations[chosen]

    def get_all_w_labels(self, label):
        assert self.test_index is not None, 'test_index is None'
        inds_to_choose = (self.Y == label).nonzero(as_tuple=True)[0]
        inds_to_choose = np.array(inds_to_choose,dtype = np.int64)
        test_graphs = [] 
        test_explanations = [] 
        for idx in inds_to_choose:
            test_graphs.append(self.graphs[idx])
            test_explanations.append(self.explanations[idx])
    ...
    def get_graph_as_networkx(self, graph_idx):
        ...
        #print(g.edges.data)
        return to_networkx_conv(g, node_attrs = ['x'], edge_attrs=['edge_attr'],to_undirected=True)
