    ...
    def explanation_generator(self, node_idx):
        ...
        for i in range(len(edge_index)):
            ...
            if self.G.has_edge(v, u):
                ...
                edge_index[i,0]=u
                edge_index[i,1]=v
        original_edge_in_num_hop =  set([self.G.edges[int(e[0]),int(e[1])]['shape'] for e in edge_index if self.G.edges[int(e[0]),int(e[1])]['shape'] == self.G.nodes[node_idx]['shape']])
...
def generate_house():
    ...
    for num in range(1000):
        if ran[num]<0.5:
            ...
            original_edge_in_num_hop =  set([G.edges[int(edge_index[0,e]),int(edge_index[1,e])]['shape'] for e in range(edge_index.shape[1]) if G.edges[int(edge_index[0,e]),int(edge_index[1,e])]['shape'] == 1])
