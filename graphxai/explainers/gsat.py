...
class GSAT(_BaseExplainer):
    ...
    def __init__(self, model: nn.Module, emb_layer_name: str = None,
                 ...
                 num_hops: int = None, in_channels = None, final_r=0.7, decay_interval=10, decay_r=0.1):
        """
        Args:
    ...
    def aggreate(self, V,edges,imp):
        
        for t in range(6):
            node_score = [0 for _ in range(V+1)]
            node_count = [0 for _ in range(V+1)]
            for i in range(len(edges)):
                edge = edges[i]
                node_score[edge[0]]+=imp[i]
                node_count[edge[0]]+=1
                node_score[edge[1]]+=imp[i]
                node_count[edge[1]]+=1
            for i in range(V+1):
                if node_count[i]!=0:
                    node_score[i] = node_score[i]/node_count[i]
            for i in range(len(edges)):
                edge = edges[i]
                imp[i] = imp[i]*0.9+(node_score[edge[0]]+node_score[edge[1]])*0.1
...
#            raise ValueError('node_idx should be provided.')
        ...
        with torch.set_grad_enabled(training):
            ...
            """
            first_node_mask = torch.sum(sym_mask,dim=1)
            
            first_edge_mask = sym_mask[edge_index[0], edge_index[1]]*0.85+(first_node_mask[edge_index[0]]+first_node_mask[edge_index[1]])*0.15
            
            
            second_mask_sparse = torch.sparse_coo_tensor(
                edge_index, first_edge_mask, (n, n))
            second_mask_sigmoid = second_mask_sparse.to_dense()
                # Undirected graph
            second_sym_mask = (second_mask_sigmoid + second_mask_sigmoid.transpose(0, 1)) / 2
            second_node_mask = torch.sum(second_sym_mask,dim=1)
            
            second_edge_mask = second_sym_mask[edge_index[0], edge_index[1]]*0.85+(second_node_mask[edge_index[0]]+second_node_mask[edge_index[1]])*0.15

            
            third_mask_sparse = torch.sparse_coo_tensor(
                edge_index, second_edge_mask, (n, n))
            third_mask_sigmoid = third_mask_sparse.to_dense()
                # Undirected graph
            third_sym_mask = (third_mask_sigmoid + third_mask_sigmoid.transpose(0, 1)) / 2
            third_node_mask = torch.sum(third_sym_mask,dim=1)
            
            edge_mask = third_sym_mask[edge_index[0], edge_index[1]]*0.85+(third_node_mask[edge_index[0]]+third_node_mask[edge_index[1]])*0.15
        ...
        #     print(tester)
            prob_with_mask = self._predict(x, edge_index, edge_weights=edge_weights,
    ...
    """
    def _loss(self, masked_pred, original_pred, mask, reg_coefs, epoch):
        """
        ...
        info_loss = (mask * torch.log(mask / r + 1e-06) + (1 - mask) * torch.log((1 - mask) / (1 - r + 1e-06) + 1e-06)).mean()
        size_loss = torch.sum(mask) * size_reg
        mask_ent_reg = -mask * torch.log(mask) - (1 - mask) * torch.log(1 - mask)
        mask_ent_loss = entropy_reg * torch.mean(mask_ent_reg)
        cce_loss = torch.nn.functional.cross_entropy(masked_pred, original_pred)
        return cce_loss + info_loss
    ...
    def train_explanation_model(self, dataset: Data, forward_kwargs: dict = {}):
        ...
        def loss_fn(prob: torch.Tensor, ori_pred: int,epoch):
            ...
            info_loss = (edge_mask * torch.log(edge_mask / r + 1e-06) + (1 - edge_mask) * torch.log((1 - edge_mask) / (1 - r + 1e-06) + 1e-06)).mean()
        ...
        if self.explain_graph:  # Explain graph-level predictions of multiple graphs
            ...
            for epoch in range(self.max_epochs):
                ...
                for gid in tqdm.tqdm(dataset_indices):
                    ...
                    loss_tmp = loss_fn(prob_with_mask.squeeze(), ori_pred_dict[gid],epoch)
                    loss_tmp.backward()
                    loss += loss_tmp.item()
