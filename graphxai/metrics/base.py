...
def connected_k(nodes, edge_index, m, k):
    states = []
    V = max(nodes)
    neighbor = [[] for _ in range(V+1)]
    for i in range(len(edge_index)):
        states.append([[i],m[i],[edge_index[i][0],edge_index[i][1]]])
        neighbor[edge_index[i][0]].append(i)
        neighbor[edge_index[i][1]].append(i)
    maxs =0 
    edges = []
    while(len(states)!=0):
        now = states[0]
        ...
        if len(now[0])<k:
            for i in now[2]:
                for j in neighbor[i]:
                    if j in now[0]:
                        continue
                    newstate=[now[0].copy(),now[1],now[2].copy()]
                    newstate[0].append(j)
                    newstate[1]+=m[j]
                    u = edge_index[j][0]
                    v = edge_index[j][1]
                    if u not in newstate[2]:
                        newstate[2].append(u)
                    if v not in newstate[2]:
                        newstate[2].append(v)
...
def graph_exp_acc(gt_exp: Explanation, generated_exp: Explanation, threshold = 0.8) -> float:
    ...
    #print(calc_node_imp)
    thresh = np.sort(calc_node_imp)[-k]
    #positives = np.argsort(-calc_node_imp)[0:k]
    TPs = []
    FPs = []
    FNs = []
    ...
    return JAC#TP/(TP+FN)#JAC

def aggreate(V,edges,imp):
    
    new_imp = np.array(imp)
    for t in range(6):
        ...
        for i in range(len(edges)):
            edge = edges[i]
            new_imp[i] = new_imp[i]*0.7+(node_score[edge[0]]+node_score[edge[1]])*0.3
    return new_imp

def graph_exp_edge_acc(gt_exp: Explanation, generated_exp: Explanation) -> float:
    '''
    Args:
        gt_exp (Explanation): Ground truth explanation from the dataset.
        generated_exp (Explanation): Explanation output by an explainer.
    ...
    #EPS = 1e-09
    #thresh = threshold
    #print(len(gt_exp.edge_imp))
    #print(len(generated_exp.edge_imp))
    relative_positives = (gt_exp.edge_imp == 1).nonzero(as_tuple=True)[0].cpu()
    #print(true_edges)
    k = len(relative_positives)
  
    #k=12
    gt_subgraph = gt_exp.enc_subgraph.edge_index.transpose(1,0)
    true_edges = [gt_subgraph[i].cpu() for i in relative_positives]
    #print(true_edges)
    # Accessing the enclosing subgraph. Will be the same for both explanation.:
    exp_subgraph = generated_exp.enc_subgraph.edge_index.transpose(1,0)
    
    calc_edge_imp = generated_exp.edge_imp.cpu()
    
    new_edge = []
    new_imp = []
    for i in range(len(exp_subgraph)):
        if exp_subgraph[i,0]<exp_subgraph[i,1]:
            new_edge.append(exp_subgraph[i])
            new_imp.append(calc_edge_imp[i])
    #print(new_edge)
    print(new_imp)
    #calc_edge = connected_k(generated_exp.enc_subgraph.nodes,new_edge,new_imp,k//2)
    
    #print(exp_subgraph)
    #V = max( generated_exp.enc_subgraph.nodes)
    #new_imp = aggreate(V,exp_subgraph,calc_edge_imp)
    #positive = np.argsort(np.array(-new_imp))[0:k]
    positive = np.argsort(np.array(-calc_edge_imp))[0:k]
    #thre = np.sort(calc_edge_imp)[-k]
    
    calc_edge = [exp_subgraph[i].cpu() for i in positive]
    #print(calc_edge)
    ...
    full=0.
    #for i in range(len(exp_subgraph)):
    for i in range(len(calc_edge)):
        #edge = exp_subgraph[i]
        #edge = new_edge[calc_edge[i]]
        edge = calc_edge[i]
        #if thre> calc_edge_imp[i]:
        #    continue
        #full+=2
        full+=1
        for j in range(len(true_edges)):
            true_edge=true_edges[j]
            if edge[0] == true_edge[0] and edge[1] == true_edge[1]:
                num+=1
            #if edge[0] == true_edge[1] and edge[1] == true_edge[0]:
            #    num+=1
    acc=num/k
    # print(f'TP / (TP+FP+FN) edge score of gnn explainer is {edge_score}')
