...
class _BaseExplainer:
    ...
    def _predict(self, x: torch.Tensor, edge_index: torch.Tensor,edge_weights=None,
        ...
        if edge_weights is None:
            ...
            with torch.set_grad_enabled(True):
                self.model.eval()
                out = self.model.to(device)(x, edge_index, edge_weights=edge_weights,**forward_kwargs)
                if return_type == 'label':
                    out = out.argmax(dim=-1)
                elif return_type == 'prob':
                    out = F.softmax(out, dim=-1)
